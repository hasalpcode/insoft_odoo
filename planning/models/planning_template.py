...
class PlanningTemplate(models.Model):
    ...
    task_id = fields.Many2one('planning.task', string="Tâche")
    ...
    def name_get(self):
        ...
        for shift_template in self:
            ...
            name = '%s - %s %s %s' % (
                ...
                shift_template.task_id.name if shift_template.task_id.name is not False else ''
            )
            result.append([shift_template.id, name])
        return result

class PlanningPreviewTemplate(models.Model):
    _name = 'planning.preview.template'
    _description = "Shift Template"

...
class PlanningRecurrency(models.Model):
    _name = 'planning.preview.recurrency'
    _description = "Planning Recurrence"

    slot_ids = fields.One2many('planning.preview', 'recurrency_id', string="Related planning entries")
    repeat_interval = fields.Integer("Répéter à chaque", default=1, required=True)
    repeat_type = fields.Selection([('forever', 'Pour toujours'), ('until', "Jusqu'à")], string='Semaine(s)', default='forever')
    repeat_until = fields.Datetime(string="Répéter jusqu'à")
    last_generated_end_datetime = fields.Datetime("Last Generated End Date", readonly=True)
    company_id = fields.Many2one('res.company', string="Company", readonly=True, required=True,
                                 default=lambda self: self.env.company)

    _sql_constraints = [
        ('check_repeat_interval_positive', 'CHECK(repeat_interval >= 1)',
         'Recurrency repeat interval should be at least 1'),
        ('check_until_limit', "CHECK((repeat_type = 'until' AND repeat_until IS NOT NULL) OR (repeat_type != 'until'))",
    ...
    def _repeat_slot(self, stop_datetime=False):
        PlanningSlot = self.env['planning.preview']
        PlanningSlot_1 = self.env['planning.slot']
        for recurrency in self:
            slot = PlanningSlot.search([('recurrency_id', '=', recurrency.id)], limit=1, order='date_start DESC')
            # for s in slot.slot_ids:
            #
            #
            #
            #     slot_values_ids = []
            #     recurrency_delta_1 = get_timedelta(recurrency.repeat_interval, 'week')
            #     next_start_s = PlanningSlot_1._add_delta_with_dst(s.start_datetime, recurrency_delta_1)
            #     recurrence_end_dt_1 = recurrency.repeat_until
            #     range_limit_1 = min([dt for dt in [recurrence_end_dt_1, stop_datetime] if dt])
            #     while next_start_s < range_limit_1:
            #         slot_ids = s.copy_data({
            #             'start_datetime': next_start_s,
            #             'end_datetime': next_start_s + (s.end_datetime - s.start_datetime),
            #             'recurrency_id': recurrency.id,
            #             'company_id': recurrency.company_id.id,
            #             'repeat': True,
            ...
            #             'is_published': False
            #         })[0]
            #         slot_values_ids.append(slot_ids)
            #         next_start_s = PlanningSlot_1._add_delta_with_dst(next_start_s, recurrency_delta_1)
            #         if slot_values_ids:
            #             PlanningSlot_1.create(slot_values_ids)
            ...
            if slot:
                ...
                # generate recurring slots
                recurrency_delta = get_timedelta(recurrency.repeat_interval, 'week')
                next_start = PlanningSlot._add_delta_with_dst(slot.date_start, recurrency_delta)

                slot_values_list = []
                while next_start < range_limit:
                    slot_values = slot.copy_data({
                        'date_start': next_start,
                        'date_stop': next_start + (slot.date_stop - slot.date_start),
                        'recurrency_id': recurrency.id,
                        'company_id': recurrency.company_id.id,
                        'repeat': True,
                ...
                if slot_values_list:
                    PlanningSlot.create(slot_values_list)
                    recurrency.write({'last_generated_end_datetime': slot_values_list[-1]['date_start']})

            else:
                recurrency.unlink()

    def _delete_slot(self, date_start):
        slots = self.env['planning.preview'].search(
            [('recurrency_id', 'in', self.ids), ('date_start', '>=', date_start), ('is_published', '=', False)])
        slots.unlink()
